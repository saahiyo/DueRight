...
db_url = os.getenv("FIREBASE_DATABASE_URL", "https://dueright-default-rtdb.asia-southeast1.firebasedatabase.app")
...
if firebase_creds_json:
    try:
        ...
        logging.info(f"Firebase Admin initialized successfully with service account pointing to {db_url}.")
...
if not firebase_inited:
    if os.getenv("K_SERVICE"):
        try:
            ...
            firebase_inited = True
            firebase_auth_enabled = True
            logging.info(f"Firebase Admin initialized successfully using default credentials pointing to {db_url}.")
        except Exception as e:
